...
async function geturls() {
  ...
  for (let b = 1; b <= 700; b++) {
    ...
    console.log('Page', b);
    try {
      await page.goto(`https://www.beymen.com/erkek-10004?sayfa=${b}`, { timeout: 0 });
      for (let i = 1; i <= 200; i++) {
      ...
      fs.appendFile("womenmenUrl.json", jsonCatch, "utf8", function (err) {
        if (err) {
  ...
  fs.appendFile("womenurl.json", jsonUrl, "utf8", function (err) {
    if (err) {
      return console.log(err);
    }
    console.log("The file was saved!");
  ...
  for (let j = 0; j < urls.length; j++) {
    ...
    try {
      ...
      const [t] = await singlepage.$x(`/html/body/div[3]/div[1]/div[1]/div[2]/div[2]/h1/a`);
      const [desc] = await singlepage.$x(`/html/body/div[3]/div[1]/div[1]/div[2]/div[2]/h1/span`);
      const [price] = await singlepage.$x(`//*[@id="priceNew"]`);
      const [oldprice] = await singlepage.$x(`//*[@id="priceOld"]`);
      let PriceVal = '',DescVal = '',titleVal = '',discVal = '';
      ...
      let Images = [];
      for (let d = 1; d <= 3; d++) {
        const [image] = await singlepage.$x(`/html/body/div[3]/div[1]/div[1]/div[2]/div[1]/figure/img[${d}]`);
        try {
      ...
      let pContent = JSON.stringify(products);
      fs.appendFile("womenproduct.json", pContent, "utf8", function (err) {
        if (err) {
          return console.log(err);
        }
        products=[];
  ...
  fs.appendFile("womenproduct.json", jsonContent, "utf8", function (err) {
    if (err) {
      return console.log(err);
    }
    console.log("The file was saved!");
